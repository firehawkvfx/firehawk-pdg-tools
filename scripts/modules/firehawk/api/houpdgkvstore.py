...
pdg_process = os.getenv('PDG_process', 0)

if not pdg_process: # PDG may load similar python libraries to houdini while outside of a houdini context.  eg in the pdgmq python process on the farm.  when that happens, we don't want to import hou, so we only conditionally load these modules when outside houdini.
    ...
    def getPdgKvStore(work_item): # Attempt to get previous job id data for a work item to retrieve a log.  this will depend on version parameters defined by hou nodes.
        ...
        if version_parm is not None:
            ...
            json_object = pdgkvstore.work_item_db_get(key, graph=work_item.graph)
